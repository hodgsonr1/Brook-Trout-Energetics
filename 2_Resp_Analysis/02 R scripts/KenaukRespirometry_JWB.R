...
#JWB plot trial data, fit models 
head(Combined_data_SMR_LabV1 %>% as.data.frame())
library(plotly)
b <- ggplot(Combined_data_SMR_LabV1 %>% filter(Fish.ID=="2024-08-22-037" & BackgroundVsTrial=="Trial"),
       aes(datetime, O2, col=as.factor(period)))+geom_point()
b
ggplotly(b) #zooming in on this to look around at slopes, looks pretty good. 

#copied the loop from below, will apply here to one fish as example:
...
#use one test fish
test.fish <- Combined_data_SMR_LabV1 %>% filter(Fish.ID=="2024-08-22-037" & BackgroundVsTrial=="Trial")
head(test.fish %>% as.data.frame())

#check out one period to make sure it's fitting properly:
ggplot(test.fish %>% filter(Fish.ID.n==76 & period==12) %>% droplevels(), aes(datetime.n, O2))+geom_point()+geom_smooth(method="lm")
...
loopC <- c()
for (i in min(test.fish$Fish.ID.n, na.rm = T):max(test.fish$Fish.ID.n, na.rm = T)){
  loopA <- filter(test.fish, F.ID.n==i)
...
background_trials <- Combined_data_SMR_LabV1 %>% 
